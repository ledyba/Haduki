...
public class OriginalNTLMScheme implements AuthScheme {
    ...
    private byte[] getType1Msg(NTCredentials cre) {
        ...
        try {
            ...
            //フラグ
            Util.writeInteger(baos, 0x00003205);
            /*ドメイン・セキュリティバッファ*/
            //長さを二回
    ...
    private byte[] getType3Msg(NTCredentials cre) {
        ...
        try {
            ...
            //len:60
            ///フラグ
            Util.writeInteger(baos, 0x00000201);
            //len:64
    ...
    public void processChallenge(String challenge) throws
        ...
        //ターゲット
        int target_length = Util.toInt(msg[0x28]) + (Util.toInt(msg[0x29]) << 8);
        int target_off = Util.toInt(msg[0x2C]) + (Util.toInt(msg[0x2d]) << 8);
